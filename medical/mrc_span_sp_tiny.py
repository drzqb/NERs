'''
    ...
    通过获取任务句的bert输出来区分 TREATMENT、BODY、SIGNS、CHECK、DISEASE
    batch_size>>5时有效
'''
...
parser.add_argument('--per_save', type=int, default=872, help='save model per num')
parser.add_argument('--check', type=str, default='model/mrc_span_sp_tiny', help='The path where model saved')
...
class BERT(keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        # 输入和任务句分次输入bert
        output = self.bert(input_ids=inputs,
                           token_type_ids=tf.zeros_like(inputs),
                           attention_mask=tf.cast(tf.greater(inputs, 0), tf.int32))[0]

        # mrc_output = []
        # for i in range(params.label_num):
        #     mrc_output.append(self.bert(input_ids=self.mrc[i:i + 1])[0][:, 0])
        #
        # mrc_output=tf.concat(mrc_output,axis=0)

        mrc_output = self.bert(input_ids=self.mrc,
                               token_type_ids=tf.zeros_like(self.mrc),
                               attention_mask=tf.cast(tf.greater(self.mrc, 0), tf.int32)
                               )[0][:, 0]
...
#
#     def call(self, inputs, **kwargs):
#         len_inputs = tf.shape(inputs)[1]
#         len_mrc = tf.shape(self.mrc)[1]
#
#         输入和任务句合并成一个新batch，大小为batch+5，数据需要对齐
#         inputs_r = tf.cond(tf.less(len_mrc, len_inputs),
#                            lambda: tf.concat(
#                                [tf.concat([self.mrc, tf.zeros([params.label_num, len_inputs - len_mrc], tf.int32)],
#                                           axis=-1), inputs], axis=0),
#                            lambda: tf.concat([self.mrc, tf.concat(
#                                [inputs, tf.zeros([tf.shape(inputs)[0], len_mrc - len_inputs], tf.int32)], axis=-1)],
#                                              axis=0)
#                            )
#
#         output = self.bert(input_ids=inputs_r,
#                            token_type_ids=tf.zeros_like(inputs_r),
#                            attention_mask=tf.cast(tf.greater(inputs_r, 0), tf.int32))[0]
#
...
class MRC(keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        x, task_embed, start, end, span, val, seqlen = inputs

        ##################################### start 和 end 的 logits  ###################################################
        # B*N*2
        startend_output = []
        for i in range(params.label_num):
            startend_output.append(self.dense_startend[i](x + task_embed[i]))

        # B*5*N*2
        startend_output = tf.stack(startend_output, axis=1)
        ...
        for i in range(params.label_num):
            # B*N*1
            xx = self.dense_ys(x + task_embed[i])

            # B*N*N*1
            startx = tf.tile(tf.expand_dims(xx, 2), [1, 1, N, 1])
            endx = tf.tile(tf.expand_dims(xx, 1), [1, N, 1, 1])
            ...
            # B*N*N
            spanx = tf.squeeze(self.dense_span(spanx), axis=-1)

            span_output.append(spanx)

        # B*5*N*N
        span_output = tf.stack(span_output, axis=1)
...
#         x_r = []
#         for i in range(params.label_num):
#             x_r.append(x + task_embed[i])
#
#         # B*5*N*768
#         x_r = tf.stack(x_r, axis=1)
#
#         # B*5*N*2
#         startend_output = self.dense_startend(x_r)
#
#         # B*10*N*1
#         startend_output_logits = tf.concat(tf.split(startend_output, 2, axis=-1), axis=1)
#
...
class USER:
    ...
    def build_model(self):
        ...
        sequence_output, task_embed = BERT(name="bert")(sen)
    ...
    def plothistory(self):
        ...
        plt.tight_layout()
        plt.savefig("mrc_span_sp_tiny_PRF.jpg", dpi=500, bbox_inches="tight")
