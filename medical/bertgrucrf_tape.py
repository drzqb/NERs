...
parser.add_argument('--check', type=str, default='model/medical_bertgrucrf_tape',
...
class BiGRU(keras.layers.Layer):
    def __init__(self, units=128, **kwargs):
        super(BiGRU, self).__init__(**kwargs)

        self.units = units

        fgru = keras.layers.GRU(units, return_sequences=True)
        self.bigru = keras.layers.Bidirectional(fgru)

    def get_config(self):
        config = super(BiGRU, self).get_config().copy()
        config.update({
            'units': self.units,
        })
...
def train_step(data, model, optimizercrf, optimizerother):
    ...
    crf_trainable_variables = [v for v in model.trainable_variables if not v.name.startswith("tf_bert")]
    other_trainable_variables = [v for v in model.trainable_variables if v.name.startswith("tf_bert")]
...
class USER:
    def build_model(self):
        ...
        bigruoutput = BiGRU(name="bigru")(inputs=(sequence_split, mask))

        predict = CRF(label_dim=params.label_dim, name="crf")(inputs=(bigruoutput, lab, seqlen, mask))
        ...
        for epoch in range(params.epochs):
            ...
            for batch, data in enumerate(train_data):
                ...
                if epoch == 0:
                    ...
                    print(
                        '\rEpoch %d/%d %d/Unknown -loss:%.5f -acc:%5.1f -precision:%5.1f -recall:%5.1f -F1:%5.1f' % (
