...
parser.add_argument('--check', type=str, default='model/medical_grucrfix', help='The path where model shall be saved')
...
class CheckCallback(tf.keras.callbacks.Callback):
    ...
    def set_params(self, params):
        super(CheckCallback, self).set_params(params)
        print(self.params)
        if 'precision_val' not in self.params['metrics']:
            self.params['metrics'].append('precision_val')
        if 'recall_val' not in self.params['metrics']:
            self.params['metrics'].append('recall_val')
        if 'F1_val' not in self.params['metrics']:
            self.params['metrics'].append('F1_val')

    def on_epoch_end(self, epoch, logs=None):
        if self.validation_data:
            logs["precision_val"]=float("inf")
            logs["recall_val"]=float("inf")
            logs["F1_val"]=float("inf")
            ...
            logs["precision_val"] = precision
            logs["recall_val"] = recall
            logs["F1_val"] = f1
