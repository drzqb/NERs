...
class BERT(Layer):
    def __init__(self, **kwargs):
        ...
        self.mrc1 = self.tokenizer.encode("找到所有的地点地址", add_special_tokens=True, return_tensors="tf")
        self.mrc2 = self.tokenizer.encode("找到所有的人物名称", add_special_tokens=True, return_tensors="tf")
        self.mrc3 = self.tokenizer.encode("找到所有的机构组织", add_special_tokens=True, return_tensors="tf")
    ...
    def call(self, inputs, **kwargs):
        ...
        mrc3 = tf.tile(self.mrc3, [batch_size, 1])
        input1 = tf.concat([mrc1, inputs[:, 1:]], axis=1)
        input2 = tf.concat([mrc2, inputs[:, 1:]], axis=1)
        input3 = tf.concat([mrc3, inputs[:, 1:]], axis=1)

        inputs_r = tf.concat([input1, input2, input3], axis=0)

        token_type_ids = tf.tile(tf.concat([tf.zeros_like(mrc1), tf.ones_like(inputs[:, 1:])], axis=1), [3, 1])
