...
parser.add_argument('--check', type=str, default='model/mrc_bertlinear_con',
...
class MRC(Layer):
    def __init__(self, label_dim, **kwargs):
        ...
        self.dense_ner = Dense(label_dim,
                               ...
                               dtype=tf.float32,
                               name='ner')
        self.task_embed = self.add_weight(name="task_embed",
                                          shape=[3, 768],
                                          dtype=tf.float32,
                                          trainable=True)
    ...
    def call(self, inputs, **kwargs):
        ...
        x1 = self.layernorm1(x + self.task_embed[0])
        x2 = self.layernorm2(x + self.task_embed[1])
        x3 = self.layernorm3(x + self.task_embed[2])
