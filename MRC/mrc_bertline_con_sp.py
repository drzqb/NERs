...
parser.add_argument('--check', type=str, default='model/mrc_bertlinear_con_sp',
...
class BERT(Layer):
    def __init__(self, **kwargs):
        ...
        self.mrc = self.tokenizer(["找到所有的地点地址",
                                   "找到所有的人物名称",
                                   "找到所有的机构组织"
    ...
    def call(self, inputs, **kwargs):
        ...
        inputs_r = tf.cond(tf.less(len_mrc, len_inputs),
                           lambda: tf.concat(
                               [tf.concat([self.mrc, tf.zeros([3, len_inputs - len_mrc], tf.int32)], axis=-1), inputs],
                               axis=0),
...
class MRC(Layer):
    ...
    def call(self, inputs, **kwargs):
        x, task_embed, label, seqlen = inputs

        x1 = x + task_embed[0]
        x2 = x + task_embed[1]
        x3 = x + task_embed[2]
        x_r = tf.concat([x1, x2, x3], axis=0)
...
class USER:
    ...
    def build_model(self):
        ...
        predict = MRC(label_dim=params.label_dim, name="mrc")(inputs=(sequence_split, task_embed, lab, seqlen))
